...
class QwenDeepResearchAgent(AgentBase):
    ...
    Args:
        ...
        memory (MemoryBase, optional):
            Memory component
        verbose (bool):
            Whether to display detailed process, defaults to False
    """
    ...
    def __init__(
        ...
        if not self.api_key:
            raise ValueError(
                "DASHSCOPE_API_KEY must be provided either as the api_key "
                "parameter or as the DASHSCOPE_API_KEY environment variable.",
            )
